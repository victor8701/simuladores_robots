...
public:
  SensorMapper() : Node("mapeo_sensores") {
    ...
    timer = this->create_wall_timer(std::chrono::milliseconds(200),
                                    std::bind(&SensorMapper::draw, this));
  ...
  void draw() {
    ...
    std::cout << COLOR_CYAN
              << "╔════════════════════════════════════════════════════════════"
                 "══════╗\033[K"
              << std::endl;
    std::cout << "║          DIAGNOSTICO DE SENSORES Y ESTADO (FSM P6)         "
                 "      ║\033[K"
              << std::endl;
    std::cout << "╚════════════════════════════════════════════════════════════"
                 "══════╝"
              << COLOR_RESET << "\033[K" << std::endl;
    ...
    auto print_sensor = [this](int i, const std::string &label) {
      ...
      snprintf(buf, sizeof(buf), "  ps%d %-7s: %s%5.2fm %s%s  ", i,
               label.c_str(), c.c_str(), ps_val[i], b.c_str(), COLOR_RESET);
      return std::string(buf);
    };

    std::cout << print_sensor(0, "(F-IZQ)") << "│" << print_sensor(7, "(F-DER)")
              << "\033[K" << std::endl;
    std::cout << print_sensor(1, "(L-45°)") << "│" << print_sensor(6, "(R-45°)")
              << "\033[K" << std::endl;
    std::cout << print_sensor(2, "(LAT-I)") << "│" << print_sensor(5, "(LAT-D)")
              << "\033[K" << std::endl;
    std::cout << print_sensor(3, "(B-IZQ)") << "│" << print_sensor(4, "(B-DER)")
              << "\033[K" << std::endl;
    ...
    // Robot Visualization
    std::cout << "               ^   "
              << (ps_val[0] < 0.8 ? "\033[31mBLOCK\033[0m"
                                  : " \033[32mOK \033[0m ")
              << " -- "
              << (ps_val[7] < 0.8 ? "\033[31mBLOCK\033[0m"
                                  : " \033[32mOK \033[0m ")
              << "   ^\033[K" << std::endl;
    std::cout << "               |      +------------------+      |\033[K"
              << std::endl;
    std::cout << "               /       |      CHASSIS     |      \\\033[K"
              << std::endl;
    std::cout << "              <---------|       2.0 m      |--------->\033[K"
              << std::endl;
    std::cout << "               \\       |       1.0 m      |      /\033[K"
              << std::endl;
    std::cout << "               |      +------------------+      |\033[K"
              << std::endl;
    std::cout << "               v                                v\033[K"
    ...
    std::cout << COLOR_WHITE << "  [ TELEMETRIA ]\033[K" << COLOR_RESET
              << std::endl;
    std::cout << std::fixed << std::setprecision(3);
    std::cout << "   X: " << std::setw(8) << pos_x
              << " m  │  Y: " << std::setw(8) << pos_y
              << " m  │  Z: " << std::setw(8) << pos_z << " m\033[K"
              << std::endl;

    std::cout << std::endl;
    std::cout << COLOR_YELLOW << "  [ LOGICA FSM ]\033[K" << COLOR_RESET
              << std::endl;
    std::cout << "   >> ESTADO: " << COLOR_WHITE << status << COLOR_RESET
              << "\033[K" << std::endl;
